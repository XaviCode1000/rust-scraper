...
| 4. Retry classification | `fetch_inner` retry loop (`wreq_downloader.rs:418`): `Timeout` = terminal (retrying a dead peer only doubles wall time); mid-body transients (`ConnectionReset`/`UnexpectedEof`) retry (#649); 429 → `max(Retry-After, exponential backoff)`; 5xx → exponential; 403 → one rotated-UA retry ONLY with unpinned UA (#503), capturing the rotated status; terminal 4xx reported as-is; retries exhausted report the LAST observed status, never a hardcoded one | Classification decides spend: whether to sleep, rotate, or fail. It runs BEFORE any body is read. |
| 5. Body read | `build_page` → `response.text()` (`wreq_downloader.rs:364`) | ⚠️ **Known gap:** the read is currently UNBOUNDED — there is no max-body-size cap on this path. Do not add fetch paths that compound this; a bounded-read stage is the intended future hardening. |
