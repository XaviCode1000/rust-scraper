...
pub const BATCH: OptionSpec = OptionSpec {
    ...
    default: Some(DefaultValue::Bool(false)),
    help: "Enable batch mode — scrape each URL from stdin (one per line), one page per URL, no crawling",
...
pub const BATCH_FILE: OptionSpec = OptionSpec {
    ...
    default: None,
    help: "Path to a file containing URLs to scrape (one per line), one page per URL, no crawling",
    heading: Some("Batch Processing"),
    kind: ValueKind::Path,
    visible_aliases: &[],
    nullable: false,
